...
class GUI:
    ...
    def twoImagesPage(self):
        ...
        def startBtnAction():
            self.isAuto = True
            self.game.findDifferencesAuto(self.image1, self.image2)
            self.gamePage()
    ...
    def resultPage(self, state=False, correctTimes=0, allTimes=0, missingRanges=None, image1=None, image2=None, originRanges=None):
    ...
    def confirmBtnAction(self):
            ...
            if not self.isAuto:
                ...
                score, mis = self.game.compareWithAuto(self.playerShapes)
            ...
            if not self.isAuto:
                ...
                self.resultPage(state, score, len(mis)+score, mis, self.image1, self.image2, self.game.autoDifferences)
    ...
    def canvas2Click(self, event):
        ...
        if not self.isAuto:
            ...
            if len(self.playerShapes) == self.game.getAutoDiffLen():
                _, mis = self.game.compareWithAuto(self.playerShapes)
                if len(mis) == 0:
                    self.confirmBtnAction()
